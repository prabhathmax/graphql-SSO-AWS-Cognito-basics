...
export class CognitoSecrets {
  ...
  static keys() {
    return ['COGNITO_POOL_ID', 'COGNITO_CLIENT_ID', 'COGNITO_REGION'];
  }

  // Returns `{ bucket: string, region: string }`;
  async get() {
    if (process.env.COGNITO_POOL_ID && process.env.COGNITO_CLIENT_ID) {
      return {
    ...
    return this.cache.get(this.key, this.ttl).then((secret) => ({
      cognitoPoolId: secret.COGNITO_POOL_ID,
      cognitoClientId: secret.COGNITO_CLIENT_ID,
      cognitoRegion: secret.COGNITO_REGION,
    }));
  }
}
